...
return new class extends Migration
{
     ...
     * 2. Ensures the is_required column exists (not 'required')
    ...
    public function up(): void
    {
        Schema::table('feedback_questions', function (Blueprint $table) {
            ...
            // Check if we need to fix the required/is_required column issue
            if (MigrationHelper::columnExists('feedback_questions', 'required') && 
                !MigrationHelper::columnExists('feedback_questions', 'is_required')) {
                // Rename required back to is_required
                $table->renameColumn('required', 'is_required');
            }
            
            // If is_required doesn't exist but required doesn't either, create is_required
